...
EXTS_DIRS = 'ai animation array clipboard config-file database file filter heightmap heightmap-physics image image-merge lobby math model model-editing model-merge network noise object parallel password physics physics-object physics-terrain reload sectors skeleton sound string terrain thread time vision watchdog'
EXTS_DIRS_GFX = 'billboard camera graphics heightmap-render input particle physics-debug render render-model render-object softbody widgets'
