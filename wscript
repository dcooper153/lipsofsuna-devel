...
EXTS_DIRS = 'ai animation config-file database file heightmap heightmap-physics image lobby math model model-editing model-merge network noise object object-physics password physics reload skeleton sound string thread tiles tiles-physics time vision watchdog'
EXTS_DIRS_GFX = 'camera graphics heightmap-render input object-render render render-model tiles-render widgets wireframe'
...
def configure(ctx):
	...
	elif not ctx.check_cfg(package='lua5.1', atleast_version='5.1', args='--cflags --libs', uselib_store='LUA', mandatory=False) and\
	     not ctx.check_cfg(package='lua', atleast_version='5.1', args='--cflags --libs', mandatory=False) and\
	     not ctx.check_cfg(package='lua5.2', atleast_version='5.2', args='--cflags --libs', uselib_store='LUA', mandatory=False):
