...
lialg_strdic_new ()
{
	...
	self = (lialgStrdic*) malloc (sizeof (lialgStrdic));
	if (self == NULL)
		return NULL;
	self->size = 0;
	self->list = NULL;
	self->tree = lialg_bst_new ((lialgBstCompare) lialg_strdic_node_compare, malloc, free);
	if (self->tree == NULL)
	{
		free (self);
		return NULL;
                        ...
                        void*          value)
{
	...
	/* Create node. */
	node = (lialgStrdicNode*)malloc (sizeof (lialgStrdicNode));
	if (node == NULL)
		return NULL;
	node->key = strdup (key);
	if (node->key == NULL)
