...
Dialogspec{name = "scapegoat death", commands = {
	{"branch generate", function(self)
		...
		local add = function(branch, var_name, quest_name)
			...
			local actor = Main.quests:get_dialog_variable(self.object, var_name .. "_mark_actor") or "the contact person"
			local marker = Main.quests:get_dialog_variable(self.object, var_name .. "_mark_marker")
		...
		local vars = Main.quests:get_dialog_variables(self.object)
		local branch = {"branch",
