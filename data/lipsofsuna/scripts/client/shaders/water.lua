...
void main()
{
	...
	vec3 x = IN.coord.xyz + vec3(0.0, 0.1*LOS_time, 0.0);
	vec3 y = IN.coord.yzx + vec3(0.0, -0.3*LOS_time, 0.0);
	float a = swirlynoise(1.0 * x) + swirlynoise(2.0 * y);
	a = pow(2.7, a);
	x = IN.coord.xyz + vec3(0.0, -0.3*LOS_time, -0.3*LOS_time);
	y = IN.coord.yzx + vec3(0.0, -0.3*LOS_time, -0.3*LOS_time);
