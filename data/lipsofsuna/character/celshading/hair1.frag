...
vec3 los_blinn_phong_hair(in vec3 lv, in vec3 ev, in vec3 ld, in vec4 eq, in vec3 normal, in float shininess);
...
void main()
{
	...
	{
		vec3 l = los_blinn_phong_hair(F_lightv[i], F_eyev, LOS_light_direction[i],
			LOS_light_equation[i], normal, LOS_material_shininess);
		diff += l.z * l.x * LOS_light_diffuse[i];
		spec += l.z * l.y * LOS_light_specular[i];
	}
	/* Hair shading. */
	float specv = diffuse.a * dot(spec.rgb, vec3(0.3, 0.59, 0.11));
	spec = mix(vec4(0.5) + spec, 0.5 * spec, diffuse.a);
	vec4 specdiff = min(LOS_material_diffuse + vec4(0.5), vec4(1.0)) * 1.5;
	diffuse *= mix(specdiff, LOS_material_diffuse, specv);
	/* Cel-shading. */
	vec3 color = los_cel_shading(diffuse, diff, spec, LOS_cel_texture);
#ifdef ENABLE_MRT
