...
Crafting.update = function(self, secs)
	...
	if not self.player then
		...
		local alchemy = Main.objects:create_object_by_spec("Obstacle", "alchemy table")
		alchemy:set_position(Vector(495,100.1,500))
		alchemy:set_visible(true)

		local anvil = Main.objects:create_object_by_spec("Obstacle", "anvil")
		...
		local workbench = Main.objects:create_object_by_spec("Obstacle", "workbench")
		workbench:set_position(Vector(505,100.1,500))
		workbench:set_visible(true)

		local chest = Main.objects:create_object_by_spec("Item", "crafting chest")
