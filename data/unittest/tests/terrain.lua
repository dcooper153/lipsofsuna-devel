local print_column = function(c)
	...
	for k,v in ipairs(c) do
		print(string.format("  Stick %d: mat=%d, hgt=%.2f", k, v[1], v[2]))
		for i=3,18,5 do
			print(string.format("    Vertex off=%.2f spl=%.1f nml=%.2f,%.2f,%.2f", v[i], v[i + 1], v[i + 2], v[i + 3], v[i + 4]))
		end
	end
end
...
Unittest:add(1, "system", "terrain: build model", function()
	...
	local x,z = t:get_nearest_chunk_with_outdated_model()
	assert(x == nil)
	assert(z == nil)
	t:load_chunk(32, 0)
	local x,z = t:get_nearest_chunk_with_outdated_model()
	...
	local m = t:build_chunk_model(1, 0)
	assert(type(m) == "table")
	local x,z = t:get_nearest_chunk_with_outdated_model()
	assert(x == nil)
	assert(z == nil)
