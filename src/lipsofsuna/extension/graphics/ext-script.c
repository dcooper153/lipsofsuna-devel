...
static void Program_render_update (LIScrArgs* args)
{
	...
	if (liscr_args_geti_float (args, 0, &secs))
		liext_graphics_update (module, secs);
}

static void Program_render_update_animations (LIScrArgs* args)
{
	float secs;
	LIExtGraphics* module;
	...
	module = liscr_script_get_userdata (args->script, LIEXT_SCRIPT_GRAPHICS);
	if (liscr_args_geti_float (args, 0, &secs))
		liren_render_update_animations (module->render, secs);
}
	...
	LIScrScript* self)
{
	...
	liscr_script_insert_cfunc (self, LISCR_SCRIPT_PROGRAM, "program_render_update_animations", Program_render_update_animations);
