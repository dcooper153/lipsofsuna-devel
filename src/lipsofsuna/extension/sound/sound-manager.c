	...
	LISndManager* self)
{
	...
	if (self->sources != NULL)
	{
		lisys_assert (self->sources->size == 0);
		lialg_ptrdic_free (self->sources);
	}
	lisys_free (self);
}
	...
	float         secs)
{
	...
	LIALG_PTRDIC_FOREACH (iter, self->sources)
	{
		source = iter.value;
		if (lisnd_source_get_stereo (source))
		{
			lisnd_source_set_position (source, &self->system->listener_position);
			lisnd_source_set_velocity (source, &self->system->listener_velocity);
		}
		lisnd_source_update (source, secs);
	}
}
