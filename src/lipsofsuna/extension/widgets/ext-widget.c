...
static void Widget_canvas_disable_scene (LIScrArgs* args)
{
	LIWdgWidget* widget;

	widget = args->self;
	liren_render_overlay_disable_scene (widget->manager->render, widget->overlay);
}

static void Widget_canvas_enable_scene (LIScrArgs* args)
{
	int i;
	int pass;
	int sort;
	...
	liren_render_overlay_enable_scene (widget->manager->render, widget->overlay,
		multisamples, hdr, viewport, &modelview, &projection, &frustum,
...
static void Widget_canvas_text (LIScrArgs* args)
{
	...
	/* Create the canvas element. */
	liwdg_widget_canvas_insert_text (widget, font, text, dest_clip_ptr, dest_position_ptr,
	...
	LIScrScript* self)
{
	...
	liscr_script_insert_mfunc (self, LIEXT_SCRIPT_WIDGET, "widget_canvas_disable_scene", Widget_canvas_disable_scene);
	liscr_script_insert_mfunc (self, LIEXT_SCRIPT_WIDGET, "widget_canvas_enable_scene", Widget_canvas_enable_scene);
