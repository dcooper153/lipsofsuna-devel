...
/**
 * \brief Serializes or deserializes the slots object.
 *
 * \param self Slots.
                       ...
                       liarcSerialize* serialize)
{
	...
	{
		/* Write header. */
		if (!liarc_writer_append_uint8 (serialize->writer, LIEXT_SLOTS_VERSION) ||
		...
		for (i = 0 ; i < self->slots.count ; i++)
		{
			...
			if (!liarc_writer_append_uint8 (serialize->writer, slot->type) ||
				!liarc_writer_append_string (serialize->writer, slot->name) ||
				!liarc_writer_append_nul (serialize->writer) ||
				!liarc_writer_append_string (serialize->writer, slot->node) ||
				!liarc_writer_append_nul (serialize->writer) ||
				!lieng_engine_write_object (self->server->engine, serialize, slot->object))
				return 0;
		}
	}
	else
	{
		...
		if (!li_reader_get_uint8 (serialize->reader, &version) ||
			...
			return 0;
		if (version != LIEXT_SLOTS_VERSION)
		{
			lisys_error_set (EINVAL, "unsupported slots version");
			return 0;
		}

		/* Read owner. */
		if (!lieng_engine_read_object (self->server->engine, serialize, &object) ||
		    !liext_slots_set_owner (self, object))
			return 0;
		...
		for (i = 0 ; i < count ; i++)
		{
			...
			node = NULL;
			slot = self->slots.array + i;
			if (!li_reader_get_uint8 (serialize->reader, &type) ||
				!li_reader_get_text (serialize->reader, "", &name) ||
				!li_reader_get_text (serialize->reader, "", &node) ||
				!lieng_engine_read_object (self->server->engine, serialize, &object))
			{
				free (name);
				free (node);
				return 0;
			}
			if (!liext_slots_insert_slot (self, type, name, node) ||
				!liext_slots_set_object (self, name, object))
			{
				free (name);
				free (node);
				return 0;
