                      ...
                      const char*     path)
{
	...
	if (path == NULL)
	{
		name = lisys_path_concat (self->module->path, "config", "generator.dat", NULL);
		if (name == NULL)
			return 0;
		reader = li_reader_new_from_file (name);
		free (name);
	}
	else
	...
	/* Read rule count. */
	if (!li_reader_get_uint32 (reader, &rules))
		goto error;

	/* Allocate rules. */
	array = calloc (rules, sizeof (liextRule*));
	if (array == NULL)
	{
	...
	{
		/* Read rule header. */
		if (!li_reader_get_float (reader, &weight) ||
		    !li_reader_get_uint32 (reader, &lines))
			goto error;
		...
		/* Allocate rule lines. */
		rule->weight = weight;
		rule->lines.array = calloc (lines, sizeof (liextRuleLine));
		if (rule->lines.array == NULL)
		{
			liext_rule_free (rule);
			lisys_error_set (ENOMEM, NULL);
			goto error;
		}
		rule->lines.count = lines;
		discard = 0;

		/* Read lines. */
		for (j = 0 ; j < lines ; j++)
		{
			line = rule->lines.array + j;
			if (!li_reader_get_float (reader, &position.x) ||
			    ...
			    !li_reader_get_text (reader, "", &name))
			{
				liext_rule_free (rule);
				goto error;
			}
			line->transform = limat_transform_init (position, rotation);
			model = lieng_engine_find_model_by_name (self->module->engine, name);
			if (model == NULL)
			{
				printf ("WARNING: rule references missing model `%s'.\n", name);
				discard = 1;
			}
			else
				line->model = model;
		...
		/* Insert or discard the rule. */
		if (discard)
		{
			printf ("WARNING: dropping rule %d.\n", i);
			liext_rule_free (rule);
		}
		else
			array[count++] = rule;
	...
	for (i = 0 ; i < self->rules.count ; i++)
		liext_rule_free (self->rules.array[i]);
	free (self->rules.array);
	self->rules.array = array;
	self->rules.count = count;

	return 1;

error:
	for (i = 0 ; i < count ; i++)
		liext_rule_free (array[i]);
	free (array);
	li_reader_free (reader);
	return 0;
}
...
liext_generator_save (liextGenerator* self)
{
	...
	/* Create writer. */
	path = lisys_path_concat (self->module->path, "config", "generator.dat", NULL);
	if (path == NULL)
		return 0;
	...
	/* Write all rules. */
	liarc_writer_append_uint32 (writer, self->rules.count);
	for (i = 0 ; i < self->rules.count ; i++)
	{
		rule = self->rules.array[i];
		liarc_writer_append_float (writer, rule->weight);
		liarc_writer_append_uint32 (writer, rule->lines.count);
		for (j = 0 ; j < rule->lines.count ; j++)
		{
			line = rule->lines.array + j;
			liarc_writer_append_float (writer, line->transform.position.x);
			liarc_writer_append_float (writer, line->transform.position.y);
			liarc_writer_append_float (writer, line->transform.position.z);
			liarc_writer_append_float (writer, line->transform.rotation.x);
			liarc_writer_append_float (writer, line->transform.rotation.y);
			liarc_writer_append_float (writer, line->transform.rotation.z);
			liarc_writer_append_float (writer, line->transform.rotation.w);
			liarc_writer_append_string (writer, line->model->name);
